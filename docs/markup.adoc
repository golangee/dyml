...
The grammar has two modes, text first (G1) and node first (G2). By default a document is written in G1, but can be switched to G2 when it starts with `#!`. See the formal grammar in link:dyml.ebnf[] for more details or the examples below to get a feeling for the grammar.
