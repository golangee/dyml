...
// A ForwardElement will be inserted as a child of the next regular element.
// They will be inserted before other children that are defined inside the element,
// so that elements appear intuitively in reading-order.
...
// ForwardAttributes will appear on the next element, instead of the current one.
